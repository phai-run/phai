-- Migration 031: rewrite views to include amount_cents (fix for v1.3.0).
--
-- Migration 029 was a no-op that left BigQuery views without `amount_cents`
-- in their projections — this is the fix for users who already applied it.
-- New installations get the correct views via the fixed 029 directly.
--
-- Mirror of schema/sqlite/028_amount_cents_views.sql. BigQuery's NUMERIC is
-- decimal-safe, so aggregation precision is unchanged. This migration adds
-- `amount_cents` to view projections for cross-backend query compatibility
-- and fixes `amount_cents > 0` display-logic parity.
--
-- Views rewritten:
--   v_transactions_effective  — adds amount_cents to projection
--   v_transactions_reportable — amount_cents comparison in dedup filter
--   v_daily_pulse             — passthrough (rebuild for source chain)
--   v_monthly_spend           — amount_cents filter
--   v_cashflow                — amount_cents-based aggregations
--   v_forecast_vs_actual      — amount_cents filter
--   v_card_summary            — open_amount_cents column
--   v_card_open_now           — open_amount_cents comparison
--   v_uncategorized           — passthrough (rebuild for dependency chain)
--
-- Aggregations stay on NUMERIC (amount) — no precision benefit from
-- switching to cents on BigQuery. The amount_cents column is included
-- for projection parity so Rust consumers can reference it uniformly.
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 2. v_transactions_reportable — amount_cents in projection + dedup filter
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. v_daily_pulse — passthrough, rebuilt for dependency chain
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 4. v_monthly_spend — amount_cents filter for cross-backend parity
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 5. v_cashflow — amount_cents-based aggregations
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 6. v_forecast_vs_actual — amount_cents filter
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 7. v_card_summary — open_amount_cents column for cross-backend parity
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 8. v_card_open_now — open_amount_cents comparison
-- ═══════════════════════════════════════════════════════════════════════════
