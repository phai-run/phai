-- Migration 029: rewrite views to include amount_cents for cross-backend
-- query compatibility.
--
-- Mirror of schema/sqlite/028_amount_cents_views.sql. BigQuery's NUMERIC is
-- decimal-safe by construction, so the SUM(CAST(amount AS REAL)) problem
-- does not exist. However, the Rust layer now writes `amount_cents` on every
-- upsert (migration 028 adds the column), and downstream consumers need it
-- in view projections for cross-backend query parity.
--
-- Views rewritten:
