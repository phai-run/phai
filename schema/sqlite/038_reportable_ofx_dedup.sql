...
-- preserved. Both dedups use LEFT JOIN anti-joins (not correlated EXISTS) so
-- the view is valid on BigQuery, which cannot de-correlate the EXISTS form
-- through the nested cashbasis/cashflow views. Anti-joins do not fan out kept
-- rows: a kept row has no matching Pluggy row, so its LEFT JOIN yields a single
-- NULL-extended row.
