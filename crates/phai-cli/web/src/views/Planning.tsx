...
const MonthPanel = ({
	...
	const realizedIn = sumAmounts(
		transactions.filter((t) => !isNegative(t.amount)).map((t) => t.amount),
	);
	...
	return (
		<Card accent={ACCENT} style={{ position: "sticky", top: 16 }}>
			...
			>
				<Stat
					label="entradas"
					value={formatMoneyNumber(realizedIn)}
					color="var(--green)"
				/>
				<Stat
					label="saídas"
					value={formatMoneyNumber(-realizedOut)}
					color="var(--rose)"
				/>
				<Stat
					label="saldo proj."
					value={formatMoneyNumber(projectedClose)}
					color="var(--purple)"
				/>
			</div>
...
const Stat = ({
	...
	>
		...
		<div
			className="mono"
			style={{ color, fontWeight: 600, fontSize: 13, marginTop: 2 }}
		>
			{value}
		</div>
	</div>
...
const Line = ({
	...
	>
		...
		<span
			className="mono"
			style={{ color: amountColor(amount), fontSize: 13, whiteSpace: "nowrap" }}
		>
			{formatMoney(amount)}
		</span>
	</div>
...
const ForecastChip = ({ forecast: f }: { forecast: ForecastView }) => {
	...
	return (
		<div
			...
			title={
				locked ? "parcela/assinatura — bloqueada" : "arraste para outro mês"
			}
			style={{
				display: "flex",
				justifyContent: "space-between",
