...
use human_format::{
    bold, brl as hf_brl, month_label, progress_bar, section_header, short_description,
};
...
async fn report_card_summary(args: CardSummaryArgs) -> Result<()> {
    ...
    for row in &rows {
        ...
        let account_label = short_description(&row.account_id, 24);
...
async fn report_card_closed_insights(args: CardClosedInsightsArgs) -> Result<()> {
    ...
    println!("{}", section_header("Fechamento por cartão"));
    ...
    // ── Top categorias ──
    println!();
    println!("{}", section_header("Top categorias"));
    ...
    // ── Assinaturas ──
    println!();
    println!("{}", section_header("Assinaturas"));
    if output.subscriptions.is_empty() {
        ...
        for row in output.subscriptions.iter().take(8) {
            println!(
                "• {} — {}/mês",
                short_description(&row.merchant_key, 28),
                hf_brl(-row.amount)
            );
        }
    }
    ...
    // ── Parcelamentos ──
    println!();
    println!("{}", section_header("Parcelamentos"));
    ...
    if n_open == 0 {
        ...
        for row in output.open_installments.iter().take(5) {
            println!(
                "• {} ({}) — {}",
                short_description(&row.merchant_key, 24),
                row.marker,
                hf_brl(-row.amount)
            );
        }
    ...
    // ── Recorrentes ──
    println!();
    println!("{}", section_header("Recorrentes"));
